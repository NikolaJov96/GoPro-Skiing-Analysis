...
def main() -> None:
    ...
    video_dir_path = args.video_directory_path
    geojson_dir_path = args.geojson_directory_path

    found_videos = {}

    # Find videos and the number of chapters
    for video_chapter_path in Path(video_dir_path).rglob('*.MP4'):
    ...
    for i, video in enumerate(found_videos.values()):
        ...
        output_file = os.path.join(geojson_dir_path, '{:04}.geojson'.format(video_id))
